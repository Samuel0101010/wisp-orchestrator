...
const PORT = Number(process.env.HARNESS_E2E_PORT ?? 4499);
...
function makeState(): { dataDir: string; repoPath: string } {
  const dataDir =
    process.env.HARNESS_E2E_DATA_DIR ?? fs.mkdtempSync(path.join(os.tmpdir(), 'harness-e2e-data-'));
  let repoPath = process.env.HARNESS_E2E_REPO_PATH;
  if (!repoPath) {
  ...
  process.env.HARNESS_E2E_DATA_DIR = dataDir;
  process.env.HARNESS_E2E_REPO_PATH = repoPath;
...
export default defineConfig({
  ...
  webServer: {
    ...
    // HARNESS_DATA_DIR rather than inheriting a polluted DB from a prior dev
    // run.
