...
- **`launch-dashboard.sh` `pick_port` no longer fails on Windows Git Bash.** Microsoft Store ships a `python3.exe` stub on `PATH` that exits non-zero with a help message on every invocation. The old `pick_port` preferred `python3` over the bash `/dev/tcp` builtin — so on Windows it tried 101 ports, the stub returned non-zero for each one, and the launcher gave up with "No free TCP port found in 4400-4500". Reordered to use `/dev/tcp` first (always available in real bash, no external tool) with python3 as a last-resort fallback for hardened bash builds. Reproduced live in the sandbox.
- **Both launchers preflight Node ≥ 20.10 before pnpm install.** Without the check, an out-of-date Node user spent 60+ seconds in `pnpm install` only to crash later with a cryptic ABI mismatch or `SyntaxError`. Now they get a clear "Node ≥ 20.10 required (found vX.Y.Z)" message in 50ms.
- **Both launchers run `corepack prepare 'pnpm@10.33.2' --activate` before delegating to `corepack pnpm`.** On Node ≥ 22 corepack prompts the user interactively to confirm the pnpm version it should download — that prompt hangs Claude Code's Bash tool, which has no stdin. The explicit `prepare --activate` makes the operation non-interactive, matching the pinned `package.json#packageManager`.
- **Both launchers verify `apps/dashboard-web/dist/index.html` exists after the build phase.** The previous post-build check only verified `apps/dashboard-server/dist/server.js`. If `vite build` failed silently (e.g. OOM, asset error), the launcher would happily start the server with `WISP_SERVE_WEB=1`; the server then crashed at first request because `dashboard-web/dist/index.html` was missing. Now the failure is surfaced from the launcher before the server starts.
- **Both launchers wait for the server to bind its port before opening the browser.** A 200ms-interval TCP probe (up to 6 seconds) replaces the prior fire-and-forget `Start-Process $url`. Without the wait, the browser opened on a connection-refused page and the user had to manually refresh after the server's cold-start migrations finished.
- **`launch-dashboard.sh` `pick_port` `/dev/tcp` fallback was inverted.** A successful connect means the port is IN USE, but the old `if ! (exec 3<>...)` condition reported it as free. On minimal containers without python3 (Alpine, BusyBox) the launcher would consistently pick the first busy port and the server immediately failed to bind. Fixed regardless of whether the new prefer-`/dev/tcp` order makes the fallback rare.
- **`.claude-plugin/marketplace.json` `plugins[].repository` removed.** The CLAUDE.md notes explicitly forbid this key (it duplicates `source` and a strict v2 schema validator may reject it), and a v2.0.19 reviewer claim that it had already been removed turned out to be wrong — it was still there. Removed for real.
- **`.claude/settings.json` `UserPromptSubmit` and `Stop` entries have `matcher: "*"`.** The verified v2 hooks schema requires the three-layer shape `{matcher, hooks: [...]}` on every entry; both blocks were missing `matcher` entirely. Without it, a strict validator may refuse to register either hook on the user's session.
- **`packages/memory-mcp/package.json` `bin` field removed.** It pointed to `./dist/server.js` which doesn't exist at `pnpm install` time (only after `pnpm build`), so pnpm always printed `WARN Failed to create bin at .../wisp-memory-mcp. ENOENT: no such file ... server.js.EXE` on every fresh install. The bin name was referenced nowhere in the codebase — the memory MCP is consumed as a library, not as an executable.
- **`apps/dashboard-server/tsconfig.json` now declares a project reference to `packages/memory-mcp`.** The runtime dep was listed in `package.json` but not in the tsconfig `references` array. Currently masked by `skipLibCheck: true` in the base config; removing skipLibCheck would have turned this into a hard TS2307. Audit B-tsconfig.
- **`commands/wisp-dashboard.md` description corrected.** Claimed "pnpm must be on PATH" — actually the launcher falls back to corepack (Node ≥ 16.13 bundled). Now also documents the OS-specific native-build prereqs (VS Build Tools on Windows, Xcode CLT on macOS, build-essential on Linux) so users who lack the C++ toolchain know to install it before reporting a "better-sqlite3 install failed" issue.

### Sandbox methodology

The release process for this version adds a fresh-clone sandbox step (not gated in CI yet): clone the public repo into a tempdir, `pnpm install --frozen-lockfile`, `pnpm build`, run the launcher end-to-end, hit `/api/health`, shut down. The sandbox caught the Windows python3-stub port-discovery bug, which neither code review nor CI would have found. The full sandbox session ran on this host before tagging; both the bash launcher and the post-build server health probe returned green (`version: "2.0.20"`, `authProbe.ok: true`).
