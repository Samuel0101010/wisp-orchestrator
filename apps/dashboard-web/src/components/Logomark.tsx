import { cn } from '@/lib/utils';

interface LogomarkProps {
  className?: string;
  /** Optional accessible label. When omitted, the SVG is decorative. */
  title?: string;
}

/**
 * WISP logomark. Single-color (`currentColor`), 24×24 viewBox,
 * sized by the consumer via `className` (e.g. `size-6`, `size-4`).
 * Inlined here so we don't depend on `vite-plugin-svgr` / `?react` imports.
 */
export function Logomark({ className, title }: LogomarkProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden={title ? undefined : true}
      role={title ? 'img' : undefined}
      aria-label={title}
      className={cn(className)}
    >
      {title && <title>{title}</title>}
      <path d="M12 1.5 18.062 5v3.5L12 5 5.938 8.5V5L12 1.5Zm9 6.062L18 9.302v7l3-1.74V7.563Zm-15 1.74L3 7.563v7l3 1.74v-7ZM12 19l-6.063-3.5V19L12 22.5 18.062 19v-3.5L12 19Zm0-12.143 4.5 2.598v5.196L12 17.143l-4.5-2.598V9.455L12 6.857Z" />
    </svg>
  );
}
