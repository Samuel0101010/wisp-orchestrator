...
if (!m) { console.error('validate-tokens.cjs: allowFiles set not found'); process.exit(1); }
const count = (m[1].match(/path\.join/g) ?? []).length;
const MAX_ALLOWED = 5;
if (count > MAX_ALLOWED) {
  console.error(`validate-tokens.cjs allowFiles grew past ${MAX_ALLOWED}: ${count}. Re-justify before raising.`);
  process.exit(1);
}
