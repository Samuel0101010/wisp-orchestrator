...
export function loadSkillFromFile(filePath: string): Skill {
  ...
  if (!fm?.name || !fm?.description || !fm?.model || !Array.isArray(fm['allowed-tools'])) {
    throw new Error(`Skill ${filePath}: invalid frontmatter (need name, description, model, allowed-tools)`);
  }
  return {
    name: fm.name,
...
export function loadAllSkills(rootDir: string): Skill[] {
  ...
  for (const e of entries) {
    ...
    if (statSync(p).isDirectory()) {
      ...
      try { out.push(loadSkillFromFile(skillFile)); }
      catch (err) { console.warn(`[skills] skipped ${skillFile}:`, err instanceof Error ? err.message : err); }
    }
  }
  return out;
}
