...
describe('writeMemoryMcpConfig', () => {
  it('writes a JSON file with mcpServers.agent-harness-memory pointing at the entrypoint', async () => {
    ...
    try {
      ...
      expect(cfg.mcpServers['agent-harness-memory']).toBeDefined();
      expect(cfg.mcpServers['agent-harness-memory'].command).toBe(process.execPath);
      expect(cfg.mcpServers['agent-harness-memory'].args).toEqual(['/path/to/server.js']);
      expect(cfg.mcpServers['agent-harness-memory'].env.HARNESS_MEMORY_DB).toBe(result.dbPath);
    } finally {
