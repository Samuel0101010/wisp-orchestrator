...
export interface RunInterviewTurnArgs {
  systemPrompt: string;
  /** The project goal the user already stated; injected as interviewer context. */
  goal?: string | null;
  current: BriefState;
...
export async function runInterviewerTurn(args: RunInterviewTurnArgs): Promise<InterviewTurnResult> {
  ...
  // Give the interviewer the project goal the user already wrote, so it doesn't
  // start cold and re-ask things the goal already answers.
  const systemPrompt =
    args.goal && args.goal.trim()
      ? `${args.systemPrompt}\n\n## Project goal (already stated by the user)\n${args.goal.trim()}\n\nUse this goal as your starting context — do NOT re-ask about details it already answers; confirm and fill in only the remaining brief fields.`
      : args.systemPrompt;
  const composed = composePrompt(systemPrompt, args.history, args.userMessage, 'user');
 ...
 */
export function renderBriefAsPrdMarkdown(
  brief: BriefState,
  projectName: string,
  goal?: string | null,
): string {
