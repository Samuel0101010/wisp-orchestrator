...
export const createWorkersRouter = (deps: WorkersRouterDeps): FastifyPluginAsync => async (app) => {
  app.get('/api/workers', wrap(async () => {
  ...
  app.post('/api/workers/:name/run', wrap(async (req, reply) => {
    const { name } = z.object({ name: z.string().min(1) }).parse(req.params);
    if (!deps.registry.get(name)) { reply.code(404); return { error: 'unknown_worker' }; }
  ...
  app.get('/api/workers/:name/runs', wrap(async (req) => {
    const { name } = z.object({ name: z.string().min(1) }).parse(req.params);
